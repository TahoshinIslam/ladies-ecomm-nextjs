 ...
 */
/**
 * The key starts with the organization.
 *
 * This cache lives in the process, and the process now reads a database
 * shared with every other store. Today one deployment serves one store, so
 * a key built only from query params cannot actually collide — but that is
 * a property of the deployment, not of this function, and `lib/tenant.js`
 * exists precisely so one process can serve many stores later. A cache
 * entry keyed "(default)" would then hand store B whatever store A asked
 * for first: the wrong catalog, rendered as if it were theirs, with nothing
 * in the logs to say so.
 *
 * Cheap to put in now, effectively impossible to notice missing later.
 */
export function getShopCacheKey(query, { isAdmin = false, organizationId } = {}) {
  ...
  const scoped = `org=${organizationId}&${canonical || "(default)"}`;
  if (scoped.length > MAX_CACHE_KEY_LENGTH) return null;
  return scoped;
}
