...
/**
 * The common case end-to-end: a raw stored catalog value, converted (if
 * still USD-denominated) at the live rate and formatted for the given
 * locale. `currency` is the transitional BDT-only-migration flag from the
 * product this value came from (see models/productModel.js's
 * `priceCurrency` / docs/CURRENCY_MIGRATION_PLAN.md) — 'BDT' means the raw
 * value is ALREADY Taka and must never be multiplied by the rate again;
 * defaults to 'USD' (the pre-migration behavior) so a call site that
 * hasn't been updated to pass a real product's currency yet fails safe
 * into the existing behavior rather than silently under-charging/
 * under-displaying a still-USD product.
 */
export function formatMoney(rawValue, locale, rate, currency = "USD") {
  const bdt = currency === "BDT" ? Math.round(Number(rawValue) || 0) : usdToBdt(rawValue, rate);
  return formatBdt(bdt, locale);
}
