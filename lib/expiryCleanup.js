...
function expiredWhereClause(organizationId) {
  // Scoped to one store. Deleting another tenant's expired rows would be
  // harmless in effect and wrong in principle: every storefront deployment
  // runs this sweep, so unscoped each one would churn through the whole
  // platform's backlog and silently remove rows it does not own.
  if (!organizationId) {
    throw new Error("expiryCleanup: an organizationId is required — the tables it sweeps are shared.");
  }
  return { sql: "organization_id = ? AND expires_at < ?", params: [organizationId, new Date()] };
}

/** Row count of `table` currently past its own expiry — read-only, no delete. */
export async function countExpired(queryFn, table, organizationId) {
  const { sql, params } = expiredWhereClause(organizationId);
 ...
 * fixed string literal (from EXPIRY_CLEANUP_TABLES below), never
 * interpolated user input, so this has no injection surface despite the
 * template-string SQL.
 */
export async function deleteExpiredInBatches(queryFn, table, organizationId, { batchSize = DEFAULT_BATCH_SIZE } = {}) {
  let total = 0;
  for (;;) {
...
export const EXPIRY_CLEANUP_TABLES = ["customer_sessions", "rate_limit_counters", "storefront_events"];
 ...
 */
export async function runExpiryCleanup(queryFn, organizationId, { batchSize } = {}) {
  const deleted = {};
  for (const table of EXPIRY_CLEANUP_TABLES) {
    deleted[table] = await deleteExpiredInBatches(queryFn, table, organizationId, { batchSize });
  }
