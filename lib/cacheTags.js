...
export const CACHE_TAGS = {
  ...
  // No admin brand-mutation route exists anywhere in this app (brand data
  // is seed-only) — this tag has no invalidation call site by design, not
  // by omission. Add one only if/when a real brand-mutation route is added.
  BRANDS: "brands",
  ATTRIBUTES: "attributes",
  PUBLIC_SETTINGS: "public-settings",
  PUBLIC_THEME: "public-theme",
  ADMIN_ANALYTICS: "admin-analytics",
  // Broad "any promotion changed" tag plus two narrower ones so a carousel
  // edit doesn't force-bust the (separately cached, audience-sensitive)
  // popup read and vice versa — see services/promotionService.js and
  // lib/serverDataCache.js's getCachedEligibleCarouselPromotions()/
  // getCachedEligiblePopupBase().
  PROMOTIONS: "promotions",
  PROMOTIONS_CAROUSEL: "promotions:carousel",
  PROMOTIONS_POPUP: "promotions:popup",
};
...
// Invalidated by the review routes on every create/update/delete, but no
// cached read currently tags itself with this (no `getCachedReviews`-style
// wrapper exists in lib/serverDataCache.js — reviews are read live). The
// invalidation calls are a harmless no-op today, kept intentionally so a
// future cached-reviews read only needs to add the matching tag here, not
// also retrofit every existing mutation site to invalidate it.
