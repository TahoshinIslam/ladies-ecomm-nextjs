...
import { getOrCreateRequestId, logEvent, FAILURE_CATEGORIES } from "./logger.js";
...
// Mirrors the old Express errorMiddleware.js's translation table (that
// file was confirmed dead and removed in Phase 6), adapted for Route
// Handlers.
function toResponse(err) {
  ...
  // Defense-in-depth fallback: every duplicate-key path in this codebase
  // (orders, payments, reviews, coupon usage) already catches its own
  // ER_DUP_ENTRY explicitly via lib/idempotency.js's isDuplicateKeyError()
  // and throws a specific, friendly HttpError instead of letting it reach
  // here. This branch only exists so an uncaught one from a future/missed
  // call site still gets a clean, generic 409 instead of falling through
  // to the raw-SQL-message 500 below — it deliberately never echoes
  // `err.sqlMessage` (which can name the table/column/value) to the
  // client. Replaces the old MongoDB-shaped `err.code === 11000` /
  // `err.keyValue` check, which nothing in this MySQL codebase throws
  // anymore.
  if (err.code === "ER_DUP_ENTRY" || err.errno === 1062) {
    console.error(err);
    return NextResponse.json({ success: false, message: "That value is already in use" }, { status: 409 });
  }
  ...
  if (err.name === "PoolExhaustedError") {
    // config/db.js's acquireConnection() — every physical DB connection was
    // busy longer than the bounded wait. Same fail-fast-with-503 shape as
    // a rate-limit-store outage below; no pool size/timeout detail reaches
    // the client, full detail is still logged server-side.
    console.error(err);
...
// Phase 11, section L — maps an error to one of lib/logger.js's bounded,
// safe FAILURE_CATEGORIES for structured server-side logging. Deliberately
// coarse (never the raw error message/class name) — see lib/logger.js's
// own comment for why a raw message could itself leak internal detail.
function categorizeError(err) {
  if (err instanceof HttpError) {
    if (err.status === 401 || err.status === 403) return FAILURE_CATEGORIES.AUTH;
    if (err.status === 404) return FAILURE_CATEGORIES.NOT_FOUND;
    if (err.status === 429) return FAILURE_CATEGORIES.RATE_LIMITED;
    if (err.status === 503) return FAILURE_CATEGORIES.DEPENDENCY_UNAVAILABLE;
    if (err.status >= 400 && err.status < 500) return FAILURE_CATEGORIES.VALIDATION;
    return FAILURE_CATEGORIES.INTERNAL;
  }
  if (err.name === "RateLimitExceededError") return FAILURE_CATEGORIES.RATE_LIMITED;
  if (err.name === "RateLimitStoreError" || err.name === "ClientIpUnavailableError" || err.name === "PoolExhaustedError") {
    return FAILURE_CATEGORIES.DEPENDENCY_UNAVAILABLE;
  }
  if (err.name === "ValidationError" || err.name === "CastError" || err.code === "ER_DUP_ENTRY" || err.errno === 1062) {
...
export function withRoute(handler) {
  return async (request, ctx) => {
    // Phase 11, section L — every request gets a correlation id: reused
    // from a platform-trusted header when shaped safely, else freshly
    // generated. Never used for authorization — purely for correlating a
    // client-visible response with the server-side structured log line
    // (and, for an error, the full-detail console.error a few lines
    // below) describing the same request.
