...
// Every seed/demo product in this environment carries a placehold.co URL
// (e.g. placehold.co/800x1000.png?text=Abaya) as a stand-in for real
// photography — a plain gray box with the category name spelled out across
// it. That reads fine as a small thumbnail (ProductCard etc.), but is a
// glaring "broken-looking" image at hero/feature scale. Callers that need a
// real photo for a large, prominent placement should treat one of these as
// "no image" (fall back to the existing `.hatch` pattern) instead of
// rendering it — this predicate is how they check.
export const isPlaceholderStub = (src) => !!src && /placehold\.co/i.test(src);
...
export const resolveVariantPricing = (product, variant) => {
  ...
  return {
    ...
    // Transitional BDT-migration flag (see models/productModel.js /
    // docs/CURRENCY_MIGRATION_PLAN.md) — every caller that formats price/
    // discountPrice/displayPrice above must pass this through to
    // settings.formatPrice()/toBdt(), never assume USD.
