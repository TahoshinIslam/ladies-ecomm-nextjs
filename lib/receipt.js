import { formatCurrency, formatDateTime, storage } from "./utils.js";
...
function buildReceiptHtml(order, { autoPrint = false } = {}) {
      ...
      (it, idx) => `
        <tr>
          ...
          <td class="desc">
            ...
            <div class="meta">Size ${escapeHtml(it.size)}</div>
          </td>
          <td class="price"><span class="pill">${formatCurrency(it.price, currency)}</span></td>
          <td class="qty"><span class="pill">${String(it.quantity).padStart(2, "0")}</span></td>
          <td class="total"><span class="pill">${formatCurrency(it.price * it.quantity, currency)}</span></td>
        </tr>`,
    )
    .join("");
...
<div class="page">
  ...
  <div class="header">
    ...
    <div class="header-right">
      ...
      <div class="invoice-meta">
        ...
        <div><strong>Invoice Date</strong> : ${escapeHtml(formatDateTime(order.createdAt || Date.now()))}</div>
        <div><strong>Total Due</strong> : ${formatCurrency(order.total || 0, currency)}</div>
      </div>
    </div>
  </div>
  ...
  <div class="bottom">
    ...
    <div class="totals">
      <div class="row"><span class="k">Sub Total</span><span>${formatCurrency(order.subtotal || 0, currency)}</span></div>
      <div class="row"><span class="k">Shipping</span><span>${(order.shippingCost || 0) === 0 ? "Free" : formatCurrency(order.shippingCost, currency)}</span></div>
      ${order.tax > 0 ? `<div class="row"><span class="k">${escapeHtml(order.taxLabel || "Tax")}</span><span>${formatCurrency(order.tax, currency)}</span></div>` : ""}
      ${order.discount > 0 ? `<div class="row"><span class="k">Discount</span><span>-${formatCurrency(order.discount, currency)}</span></div>` : ""}
      <div class="row grand"><span class="k">Total</span><span>${formatCurrency(order.total || 0, currency)}</span></div>
    </div>
  </div>
...
export function downloadReceipt(order) {
  if (!order) return;
  const { html, orderNum } = buildReceiptHtml(order, { autoPrint: false });
...
export function printReceipt(order) {
  if (!order) return;
  const { html } = buildReceiptHtml(order, { autoPrint: true });
  const blob = new Blob([html], { type: "text/html" });
  const url = URL.createObjectURL(blob);
