...
// Duck-types a MySQL/MariaDB duplicate-key error (ER_DUP_ENTRY / errno 1062
// — the direct equivalent of MongoDB's old error code 11000), optionally
// narrowed to a specific unique index by name. mysql2 surfaces the violated
// index inside the error's `sqlMessage` text (e.g. "... for key
// 'orders.uq_orders_customer_idempotency'"), not as a separate structured
// field, so narrowing matches on that substring.
export function isDuplicateKeyError(err, indexName) {
  ...
  return typeof err.sqlMessage === "string" && err.sqlMessage.includes(indexName);
}
