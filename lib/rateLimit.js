...
//
// WHY MONGODB, NOT REDIS: this codebase has `config/redis.js`,
...
// is NOT in package.json's dependencies at all, `express-rate-limit` and
// `rate-limit-redis` (imported by app/middleware/rateLimiter.js) are also
// not installed, and that file even imports a `../utils/logger.js` that
// does not exist anywhere in the repository. None of those three files
// are imported by anything else, confirmed by a repo-wide grep. Redis is
// not installed, not configured, not connected, and therefore cannot be
// claimed as this application's rate-limit backend. Per the Phase 3
// instructions, this is a fresh, narrowly-scoped module built on the
// MongoDB infrastructure this codebase already tests against — not a
// reuse of that dead Express middleware (left untouched; its removal
// belongs to the later dead-code-cleanup phase), and not a new service
// dependency installed just to look more sophisticated.
//
