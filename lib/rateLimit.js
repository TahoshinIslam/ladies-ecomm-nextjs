...
export async function checkRateLimit({ identity, action, limit, windowMs }) {
  ...
  // A little past the window's own end, so a request arriving right at
  // the boundary still finds a live (not-yet-cleaned-up) row; this is a
  // storage-reclamation backstop only, never relied on for correctness.
  const expiresAt = new Date(windowStart.getTime() + windowMs + 60_000);

  let count;
  try {
    // Single atomic `INSERT ... ON DUPLICATE KEY UPDATE` — unlike the old
    // Mongo upsert, MySQL never raises a duplicate-key error for this
    // statement shape, so there's no separate "lost the race, retry as a
    // plain increment" branch needed (see models/rateLimitModel.js).
    count = await RateLimitCounter.upsertAndIncrement({ keyHash, action, windowStart, expiresAt });
  } catch (err) {
