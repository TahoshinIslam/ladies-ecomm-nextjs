 ...
 */
export async function getCachedProductList(query, cacheKey) {
...
export async function getCachedCategories() {
  return unstable_cache(
    async () => serializeForClient(await listCategories()),
    ["categories"],
...
export async function getCachedBrands() {
  return unstable_cache(
    async () => serializeForClient(await Brand.find({ isActive: true }).sort("name").lean()),
    ["brands"],
...
export async function getCachedAttributesForCategory(topCategoryId) {
  return unstable_cache(
    async () => serializeForClient(await resolveAttributesForCategory(topCategoryId)),
...
export async function getCachedAllAttributes() {
  return unstable_cache(
    async () => serializeForClient(await listAttributes()),
    ["attributes-all"],
    { revalidate: CACHE_TTL_SECONDS.ATTRIBUTES, tags: [CACHE_TAGS.ATTRIBUTES] },
  )();
}
...
export async function getCachedSitemapProducts() {
  return unstable_cache(
    async () =>
      serializeForClient(
        await Product.find({ isActive: true }).select("slug updatedAt").lean(),
      ),
    ["sitemap-products"],
    { revalidate: CACHE_TTL_SECONDS.PRODUCT_LIST, tags: [CACHE_TAGS.CATALOG] },
  )();
...
export async function getCachedPublicSettings() {
  return unstable_cache(
    async () => serializeForClient(await getPublicSettings()),
    ["public-settings"],
...
export async function getCachedActiveTheme() {
  return unstable_cache(
    async () => serializeForClient(await getActiveTheme()),
    ["public-theme"],
...
export async function getCachedOverview() {
  return unstable_cache(
    async () => serializeForClient(await getOverview()),
    ["admin-overview"],
...
export async function getCachedSalesSeries(days) {
  return unstable_cache(
    async () => serializeForClient(await getSalesSeries(days)),
...
export async function getCachedTopProducts(limit) {
  return unstable_cache(
    async () => serializeForClient(await getTopProducts(limit)),
...
export async function getCachedStatusBreakdown() {
  return unstable_cache(
    async () => serializeForClient(await getStatusBreakdown()),
...
export async function getCachedRevenueByMethod() {
  return unstable_cache(
    async () => serializeForClient(await getRevenueByMethod()),
