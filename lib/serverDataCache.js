...
import { getEligiblePromotionsBase } from "../services/promotionService.js";
...
export const CACHE_TTL_SECONDS = {
  ...
  PUBLIC_THEME: 900,
  ADMIN_ANALYTICS: 45,
  // Short — mutation invalidation (an admin create/pause/schedule) is the
  // real freshness mechanism (see app/api/admin/promotions/*/route.js);
  // this TTL only bounds staleness if an invalidation call ever silently
  // fails, and a promotion banner being briefly stale is low-stakes
  // compared to price/stock.
  PROMOTIONS: 60,
};
...
export async function getCachedBrands() {
  return unstable_cache(
    async () => withDb(async () => serializeForClient(await Brand.findActive())),
    ["brands"],
...
export async function getCachedSitemapProducts() {
  return unstable_cache(
    async () =>
      withDb(async () => serializeForClient(await Product.findSitemapEntries())),
    ["sitemap-products"],
    { revalidate: CACHE_TTL_SECONDS.PRODUCT_LIST, tags: [CACHE_TAGS.CATALOG] },
  )();
}
...
// Deliberately audience-BLIND (see services/promotionService.js's own
// comment) — the route calling this (app/api/promotions/carousel/route.js
// or .../popup/route.js) filters the returned array by the requesting
// visitor's own session AFTER this cache read resolves. Never call this
// with anything derived from a session/cookie as part of the cache key.
export async function getCachedEligiblePromotions(type, placement, pageScope) {
  const tag = type === "popup" ? CACHE_TAGS.PROMOTIONS_POPUP : CACHE_TAGS.PROMOTIONS_CAROUSEL;
  return unstable_cache(
    async () => withDb(async () => getEligiblePromotionsBase({ type, placement, pageScope })),
    ["eligible-promotions", type, placement, pageScope],
    { revalidate: CACHE_TTL_SECONDS.PROMOTIONS, tags: [CACHE_TAGS.PROMOTIONS, tag] },
  )();
}
