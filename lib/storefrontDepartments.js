...
//
// Matched by slug alone, not "is this a root category" — FASHION_DEPARTMENT
// _SLUGS members now sit one level under the Men/Women gender divisions
// (parent set), and this curated row must keep showing them individually
// exactly as before. STOREFRONT_DEPARTMENT_SLUGS is itself already a real,
// curated allowlist (not "any root category"), so slug membership alone is
// still a safe filter — a stray/test category was never going to match one
// of these exact slugs regardless of its depth.
export function sortDepartmentsForFavourites(categories) {
  return categories
    .filter((c) => STOREFRONT_DEPARTMENT_SLUGS.includes(c.slug))
    .sort((a, b) => {
