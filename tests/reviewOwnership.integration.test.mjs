...
//
// Requires MONGO_URI_TEST + JWT_SECRET in the test environment. Skips
// (never fails) if either is missing — see tests/helpers/testDb.mjs.
...
describe("review ownership — PUT/DELETE /api/reviews/[id], POST helpful/reply", { skip: !canRun && reason }, () => {
  ...
  test("non-owner customer is rejected on PUT (403)", async () => {
    ...
    const req = requestAs({
      method: "PUT",
      url: `http://test/api/reviews/${review._id}`,
      token: signTestToken(otherCustomer._id),
  ...
  test("non-owner customer is rejected on DELETE (403), review survives", async () => {
    const { review } = await makeReview();
    const req = requestAs({ method: "DELETE", url: `http://test/api/reviews/${review._id}`, token: signTestToken(otherCustomer._id) });
  ...
  test("owner can delete their own review (DELETE)", async () => {
    const { review } = await makeReview();
    const req = requestAs({ method: "DELETE", url: `http://test/api/reviews/${review._id}`, token: signTestToken(owner._id) });
    const res = await DELETE(req, { params: Promise.resolve({ id: review._id.toString() }) });
    assert.equal(res.status, 200);
  ...
  test("admin can delete someone else's review (DELETE)", async () => {
    const { review } = await makeReview();
    const req = requestAs({ method: "DELETE", url: `http://test/api/reviews/${review._id}`, token: signTestToken(admin._id) });
    const res = await DELETE(req, { params: Promise.resolve({ id: review._id.toString() }) });
    assert.equal(res.status, 200);
  });
  ...
  test("nonexistent (but valid-format) review id returns 404 on PUT/DELETE", async () => {
    ...
    const putReq = requestAs({ method: "PUT", url: `http://test/api/reviews/${fakeId}`, token: signTestToken(owner._id), body: { comment: "x" } });
    const putRes = await PUT(putReq, { params: Promise.resolve({ id: fakeId }) });
    assert.equal(putRes.status, 404);

    const delReq = requestAs({ method: "DELETE", url: `http://test/api/reviews/${fakeId}`, token: signTestToken(owner._id) });
  ...
  test("malformed (non-ObjectId) review id returns 404, not 500 — lib/http.js's CastError mapping", async () => {
    const req = requestAs({ method: "PUT", url: "http://test/api/reviews/not-a-valid-id", token: signTestToken(owner._id), body: { comment: "x" } });
  ...
  test("a stranger (non-owner, non-staff) can mark a review helpful — no ownership check exists on this endpoint", async () => {
    const { review } = await makeReview();
    const req = requestAs({ method: "POST", url: `http://test/api/reviews/${review._id}/helpful`, token: signTestToken(otherCustomer._id) });
  ...
  test("the review's own author CAN mark their own review helpful — nothing prevents it", async () => {
    const { review } = await makeReview();
    const req = requestAs({ method: "POST", url: `http://test/api/reviews/${review._id}/helpful`, token: signTestToken(owner._id) });
    const res = await helpfulPOST(req, { params: Promise.resolve({ id: review._id.toString() }) });
    assert.equal(res.status, 200);
    const json = await res.json();
  ...
  test("markHelpful on a nonexistent review returns 404", async () => {
    ...
    const req = requestAs({ method: "POST", url: `http://test/api/reviews/${fakeId}/helpful`, token: signTestToken(owner._id) });
  ...
  test("reply requires REVIEWS_MANAGE — plain customer (including the review's own owner) gets 403", async () => {
    ...
    const req = requestAs({
      ...
      token: signTestToken(owner._id), // the review's own author — still just a customer
  ...
  test("reply requires REVIEWS_MANAGE — employee WITHOUT that permission gets 403", async () => {
    ...
    const req = requestAs({
      method: "POST",
      url: `http://test/api/reviews/${review._id}/reply`,
      token: signTestToken(employeeWithoutReviews._id),
  ...
  test("reply succeeds for an employee WITH REVIEWS_MANAGE, and for admin", async () => {
    ...
    const req = requestAs({
      method: "POST",
      url: `http://test/api/reviews/${review._id}/reply`,
      token: signTestToken(employeeWithReviews._id),
