...
/**
 * Every aggregate here is scoped to this store.
 *
 * An unscoped COUNT or SUM does not fail or look empty — it returns a
 * plausible number that happens to be the whole platform's. This file
 * previously counted shoppers with `SELECT COUNT(*) FROM users`, which in
 * the shared database is not even the right table: `users` holds dashboard
 * staff, and shoppers are `customers`.
 */
  ...
  const [
    ...
    query(
      `SELECT COALESCE(SUM(total), 0) AS total FROM orders
        WHERE organization_id = ? AND deleted_at IS NULL AND status IN (${revenueStatusPlaceholders()})`,
      [org, ...REVENUE_STATUSES],
    ),
    query(
      `SELECT COALESCE(SUM(total), 0) AS total FROM orders
        WHERE organization_id = ? AND deleted_at IS NULL
          AND status IN (${revenueStatusPlaceholders()}) AND created_at >= ?`,
      [org, ...REVENUE_STATUSES, startOfMonth],
    ),
    query("SELECT COUNT(*) AS n FROM customers WHERE organization_id = ? AND deleted_at IS NULL", [org]),
    query(
      "SELECT COUNT(*) AS n FROM customers WHERE organization_id = ? AND deleted_at IS NULL AND created_at >= ?",
      [org, last30],
    ),
    query(
      "SELECT COUNT(*) AS n FROM products WHERE organization_id = ? AND deleted_at IS NULL AND is_active = 1",
      [org],
    ),
    ...
    query(
      "SELECT COUNT(*) AS n FROM orders WHERE organization_id = ? AND deleted_at IS NULL AND status = 'pending'",
      [org],
    ),
    query("SELECT AVG(rating) AS avg FROM reviews WHERE organization_id = ? AND deleted_at IS NULL", [org]),
  ]);
...
export async function getSalesSeries(days = 30) {
  ...
  const rows = await query(
    `SELECT DATE_FORMAT(created_at, '%Y-%m-%d') AS date, COALESCE(SUM(total), 0) AS revenue, COUNT(*) AS orders
     FROM orders
     WHERE organization_id = ? AND deleted_at IS NULL
       AND created_at >= ? AND status IN (${revenueStatusPlaceholders()})
     GROUP BY DATE_FORMAT(created_at, '%Y-%m-%d')
     ORDER BY date ASC`,
    [getOrganizationId(), since, ...REVENUE_STATUSES],
  );
  ...
  const rows = await query(
    `SELECT oi.product_id AS _id,
     ...
     WHERE o.organization_id = ? AND o.deleted_at IS NULL
       AND o.status IN (${revenueStatusPlaceholders()})
     GROUP BY oi.product_id
     ORDER BY totalSold DESC
     LIMIT ?`,
    [getOrganizationId(), ...REVENUE_STATUSES, clamped],
  );
...
export async function getRevenueByMethod() {
  const rows = await query(
    `SELECT COALESCE(p.method, 'unknown') AS method, SUM(o.total) AS revenue, COUNT(*) AS count
     FROM orders o
     LEFT JOIN payments p ON p.order_id = o.id AND p.organization_id = o.organization_id
     WHERE o.organization_id = ? AND o.deleted_at IS NULL
       AND o.status IN (${revenueStatusPlaceholders()})
     GROUP BY COALESCE(p.method, 'unknown')`,
    [getOrganizationId(), ...REVENUE_STATUSES],
  );
