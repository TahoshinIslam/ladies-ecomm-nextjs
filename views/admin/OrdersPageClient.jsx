...
const upsertGetAllOrders = (args, data) => shopApi.util.upsertQueryData("getAllOrders", args, data);

// `initialData`/`initialParams` come from views/admin/OrdersPage.jsx (a real
// Server Component) — the exact same {page, limit, search, sortBy,
// sortOrder, status} shape useTableQueryState()/useGetAllOrdersQuery()
// below compute from this same URL, on this same first render, via
// lib/adminTableParams.js's server-side mirror of that same URL-parsing
// logic. useHydratedQuery() seeds the RTK Query cache from initialData for
// exactly that args combination, so this component's own first fetch never
// duplicates the read the server already did — verified in
// tests/adminOrdersServerHydration.test.mjs.
export default function AdminOrdersPage({ initialData, initialParams }) {
  ...
  // initialData only ever matches the args this page was actually
  // server-rendered with (initialParams) — if the shopper has already
  // navigated to a different page/filter since then, this correctly stops
  // applying and useGetAllOrdersQuery fetches normally.
  const argsMatchInitial = JSON.stringify(queryArgs) === JSON.stringify(initialParams);
  const { data, isLoading, isFetching, isError, error, refetch } = useHydratedQuery(
    useGetAllOrdersQuery,
    queryArgs,
    argsMatchInitial ? initialData : undefined,
    upsertGetAllOrders,
  );
