...
import { useHydratedQuery } from "../../hooks/useHydratedQuery.js";
...
const upsertGetCategories = (args, data) => shopApi.util.upsertQueryData("getCategories", args, data);
const upsertGetAttributes = (args, data) => shopApi.util.upsertQueryData("getAttributes", args, data);

// `initialCategories`/`initialAttributes` come from views/admin/CategoriesPage.jsx
// (a real Server Component) — see views/admin/OrdersPageClient.jsx's own
// comment for the full hydration-handoff explanation. Both
// getCategories()/getAttributes() here are whole-collection, no-argument
// queries (undefined args), so there's no per-page-args matching needed —
// every render of this page uses the exact same cache key.
export default function AdminCategoriesPage({ initialCategories, initialAttributes }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {tab === "categories" ? (
        <CategoryTree initialCategories={initialCategories} />
      ) : (
        <AttributeManager initialAttributes={initialAttributes} initialCategories={initialCategories} />
      )}
    </div>
  );
}
...
function CategoryTree({ initialCategories }) {
  const { data, isLoading } = useHydratedQuery(useGetCategoriesQuery, undefined, initialCategories, upsertGetCategories);
...
function AttributeManager({ initialAttributes, initialCategories }) {
  // no category param -> full raw list
  const { data, isLoading } = useHydratedQuery(useGetAttributesQuery, undefined, initialAttributes, upsertGetAttributes);
  const { data: catsData } = useHydratedQuery(useGetCategoriesQuery, undefined, initialCategories, upsertGetCategories);
